...
def voting_procedure(problem: Problem, solutions, votes_idxs: dict[str, float]) -> SolverResults:
    ...
    # TODO: make according to what GNIMBUS think is the best, select the last one
...
def solve_intermediate_solutions_only_objs(  # noqa: PLR0913
    ...
    for rp in reference_points:
        ...
        # add_asf =
        add_asf = add_group_asf_MALAGAFIX

        asf_problem, target = add_asf(problem, "target", rp, delta_scal, **(scalarization_options or {}))
...
def solve_sub_problems(  # noqa: PLR0913
    ...
    achievable_prefs = [ind_sols[0].optimal_objectives, ind_sols[1].optimal_objectives,
                        ind_sols[2].optimal_objectives, ind_sols[3].optimal_objectives]  # TODO: loop to number of DMs
    aspirations, hard_constraints = find_min_max_values(achievable_prefs, problem)
    ...
    if phase == "decision":
        ...
        problem_g_nimbus, gnimbus_target = add_nimbus_sf(
            problem, "nimbus_sf", classification_list, delta, hard_constraints, current_objectives, **(scalarization_options or {})
        )

        if _solver_options:
            gnimbus_solver = init_solver(problem_g_nimbus, _solver_options)  # type:ignore
        else:
            gnimbus_solver = init_solver(problem_g_nimbus)

...
def test_add_group_asf_diff(GRP: dict[str, float], prefs: list[dict[str, float]]):
...
def test_add_group_guess_diff(GRP: dict[str, float], prefs: list[dict[str, float]]):
    """Test that the multiple decision maker achievement scalarizing function works with one reference point."""
    n_variables = 30
    n_objectives = 2
    problem = zdt1(n_variables)
...
def test_add_group_stom_diff(GRP: dict[str, float], prefs: list[dict[str, float]]):
    """Test that the multiple decision maker achievement scalarizing function works with one reference point."""
    n_variables = 30
    n_objectives = 2
    problem = zdt1(n_variables)
...
def plot2d():
    ...
    # fix this mess
    all_prefs = reference_points # !! is assumed to be list or smh in test_add_group_asf_diff jne
    print(all_prefs)
    ...
    asf, group_asf_sol = test_add_group_asf_diff(GRP, all_prefs)
    guess, group_guess_sol = test_add_group_guess_diff(GRP, all_prefs)
    stom, group_stom_sol = test_add_group_stom_diff(GRP, all_prefs)
...
# TODO fix the mess!!
    ...
    """
    all_solutions = [
        [s["f_1"] for s in solutions],
        [s["f_2"] for s in solutions]
    ]
    """
    all_solutions = [{'f_1': entry['f_1'], 'f_2': entry['f_2']} for entry in all_solutions]
...
def test_maxmin():
    ...
    print(all_points)
    problem = get_problem("zdt1")
    print(type(problem.pareto_front()))
...
if __name__ == "__main__":
    ...
    asf_sol, group_asf_sol = test_add_group_asf_diff(rp, grp)
    guess_sol, group_guess_sol = test_add_group_guess_diff(rp, grp)
    stom_sol, group_stom_sol = test_add_group_stom_diff(rp, grp)
    ...
    ax.set(xlabel='f_1', ylabel='f_2',title='Group scalarization')
    ax.grid()
    ax.legend()

    #fig.savefig("gscalares/test9.png")
    plt.show()

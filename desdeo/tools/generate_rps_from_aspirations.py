"""Generate IPR reference points inside the convex hull of a set of aspiration points.

In the "favourite" method (and similar group decision making methods) each decision maker (DM) has a corresponding
aspiration point, and the convex hull of these points is the region of compromise between them. The functions here
sample reference points uniformly inside that hull, so that the iterative Pareto representer
(`desdeo.tools.iterative_pareto_representer`) only searches for solutions between what the DMs asked for.
